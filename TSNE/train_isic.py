...
# torch.save(model, root_path+'/model/efficientnetb3_model.pth') 
...
if 'HML' in args.loss_type:
    args.store_name = '_'.join(['isic2019', 'efficientb3', args.loss_type, 'W' ,args.weighting_type,'F',str(args.ratio_finetune),'Dis',str(args.distribution),'D',str(args.delta),'start_hm_type',args.hm_delay_type])
else:
    args.store_name = '_'.join(['isic2019', 'efficientb3', args.loss_type, 'W' ,args.weighting_type,'F',str(args.ratio_finetune),'Dis',str(args.distribution)])
...
for run in run_list:
    ...
    for epoch in tqdm(range(1, epoch_num+1)):
        ...
        if args.loss_type == 'CE':
            ...
            criterion_hm = IBLoss(weight=per_cls_weights).to(device)
        ...
        if val_bacc>=best_val_bacc:
            ...
            patience -= 1
            if patience<=args.delay_hm and args.hm_delay_type=='delay':
                args.hml=True
                print('------------------------------------------------------------')
                print('training with HMLoss on [epoch %d]' % (epoch))
                print('------------------------------------------------------------')
            if patience == 0:
